...
mutates(ListWorkspaceMembersTool::class);
mutates(WorkspaceMembersContext::class);
...
it('lists only the current workspace members with id, name, and email', function (): void {
    ...
    $payload = json_decode(resolve(ListWorkspaceMembersTool::class)->handle(new Request([])), true);
...
it('filters members by name or email search', function (): void {
    ...
    $byName = json_decode(resolve(ListWorkspaceMembersTool::class)->handle(new Request(['search' => 'Searchable'])), true);
    $byEmail = json_decode(resolve(ListWorkspaceMembersTool::class)->handle(new Request(['search' => 'sam@example'])), true);
