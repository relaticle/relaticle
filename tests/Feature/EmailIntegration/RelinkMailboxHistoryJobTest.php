...
it('creates a person from stored outbound mail after the workspace switches to Selective', function (): void {
    ...
    expect($email->fresh()->linked_at)->not->toBeNull()
        ->and(People::query()->where('workspace_id', $this->workspace->id)->where('name', 'New Prospect')->exists())->toBeFalse();
    ...
    expect(People::query()->where('workspace_id', $this->workspace->id)->where('name', 'New Prospect')->exists())->toBeTrue();
});
...
it('creates a person from a stored meeting after the workspace switches to Selective', function (): void {
    ...
    expect(People::query()->where('workspace_id', $this->workspace->id)->count())->toBe(0);
    ...
    expect(People::query()->where('workspace_id', $this->workspace->id)->count())->toBe(1);
});
...
it('does not relink mail stored on another connected account', function (): void {
    ...
    expect(People::query()->where('workspace_id', $this->workspace->id)->where('name', 'Other Mailbox Prospect')->exists())->toBeFalse();
});
