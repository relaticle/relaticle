...
it('relabels cancel once copying the invite link has created the workspace', function (): void {
    ...
    expect($component->instance()->getCancelLabel())
        ->toBe(__('filament/pages/workspaces.create_workspace.actions.cancel'));

    $component->callAction(TestAction::make('copyInviteLink')->schemaComponent());

    $workspace = Workspace::query()->where('name', 'Pre Created')->firstOrFail();

    expect($component->instance()->getCancelLabel())
        ->toBe(__('filament/pages/workspaces.create_workspace.actions.go_to_workspace'))
        ->and($component->instance()->getCancelUrl())
        ->toBe(Dashboard::getUrl(['tenant' => $workspace]));
});
...
it('reconciles name and slug edited after the invite link pre-created the workspace', function (): void {
    ...
    $workspace = Workspace::query()->where('name', 'First Name')->firstOrFail();
...
it('labels the submit button for what it will actually do', function (): void {
    ...
    $component
        ...
        ->assertDontSee(__('filament/pages/workspaces.create_workspace.actions.send_invites'));
    ...
    $component
        ->assertSee(__('filament/pages/workspaces.create_workspace.actions.send_invites'))
...
it('does not flag the workspace created event when the invite link only pre-creates the workspace', function (): void {
    ...
    expect(Workspace::query()->where('name', 'Abandoned Corp')->exists())->toBeTrue()
