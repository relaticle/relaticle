...
function acceptUrlFor(WorkspaceInvitation $invitation): string
{
    $rawToken = $invitation->issueToken();
    $invitation->save();

    return route('workspace-invitations.token.accept', ['token' => $rawToken]);
}

test('guest clicking a workspace invitation link is redirected to the login page', function (bool $accountExists): void {
...
test('guest with a well-formed token for a since-deleted invitation is sent to login', function (): void {
    $acceptUrl = route('workspace-invitations.token.accept', ['token' => Str::random(40)]);

    $this->get($acceptUrl)
        ->assertRedirect(Filament::getLoginUrl());
});

test('guest clicking invitation link sees the workspace name banner on the login page', function () {
    $workspace = Workspace::factory()->create(['name' => 'Acme Corp']);
