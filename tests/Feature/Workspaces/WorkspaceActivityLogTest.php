...
beforeEach(function (): void {
    $this->owner = User::factory()->withWorkspace()->create(['name' => 'Ada Owner']);
...
test('an admin sees who deleted a record and when', function (): void {
    $company = Company::factory()->for($this->workspace)->create(['name' => 'Lumen Robotics']);
    $company->delete();
...
test('a record that still exists links back to it, a destroyed one does not', function (): void {
    $alive = Company::factory()->for($this->workspace)->create(['name' => 'Alive Corp']);
    $alive->delete();

    $destroyed = Company::factory()->for($this->workspace)->create(['name' => 'Vanished Corp']);
...
test('it lists deletions across every crm record type', function (string $factory, string $name): void {
    $record = $factory::factory()->for($this->workspace)->create(['name' => $name]);
    $record->delete();
...
test('another workspace activity never leaks in', function (): void {
    $stranger = User::factory()->withWorkspace()->create(['name' => 'Mallory Stranger']);
    $strangerWorkspace = $stranger->currentWorkspace;

    $this->actingAs($stranger);
    Filament::setTenant($strangerWorkspace);
    Company::factory()->for($strangerWorkspace)->create(['name' => 'Foreign Holdings'])->delete();
...
test('a member without the admin role cannot open the audit log', function (): void {
    ...
    $this->get(ActivityLog::getUrl(tenant: $this->workspace))->assertForbidden();
});
...
test('one save is one row, even when custom fields log their own event', function (): void {
    $company = Company::factory()->for($this->workspace)->create(['name' => 'Batched Co']);
...
test('one batch spanning two records keeps each record its own row and payload', function (): void {
    $company = Company::factory()->for($this->workspace)->create(['name' => 'Batch Company']);
    $person = People::factory()->for($this->workspace)->create(['name' => 'Batch Person']);
...
test('a create and a delete in one request both stay on the record', function (): void {
    $company = Company::factory()->for($this->workspace)->create(['name' => 'Short Lived Co']);
    $batch = (string) Str::uuid();
    ...
    livewire(ActivityLog::class)
        ->assertOk()
        ->assertCanSeeTableRecords($rows->all())
        ->assertSee(__('workspaces.activity.events.created'))
        ->assertSee(__('workspaces.activity.events.deleted'));
});
...
test('a save touching several custom fields shows every one of them', function (): void {
    $company = Company::factory()->for($this->workspace)->create(['name' => 'Many Fields Co']);
    $batch = (string) Str::uuid();
...
test('custom field rows with no native sibling collapse without borrowing each others diffs', function (): void {
    $company = Company::factory()->for($this->workspace)->create(['name' => 'Field Only Co']);
    $batch = (string) Str::uuid();
...
test('a stale morph alias whose model is gone does not take the page down', function (): void {
    ...
    Company::factory()->for($this->workspace)->create(['name' => 'Neighbour Co'])->delete();
...
test('renaming a record does not rewrite its history', function (): void {
    $company = Company::factory()->for($this->workspace)->create(['name' => 'Original Name Ltd']);
...
test('it spells out what actually changed on a row', function (): void {
    $company = Company::factory()->for($this->workspace)->create(['name' => 'Diff Co']);
...
test('a shared filtered url renders already filtered', function (): void {
    Company::factory()->for($this->workspace)->create(['name' => 'Still Here']);
    Company::factory()->for($this->workspace)->create(['name' => 'Gone Already'])->delete();
...
test('a custom field edit reads as an update, not its raw event name', function (): void {
    $company = Company::factory()->for($this->workspace)->create(['name' => 'Field Edited Co']);
    ...
    livewire(ActivityLog::class)
        ...
        ->assertSee(__('workspaces.activity.events.updated'))
        ->assertDontSee('custom_field_changes');
});

test('filtering on updated covers custom field edits too', function (): void {
    $company = Company::factory()->for($this->workspace)->create(['name' => 'Field Edited Co']);
    Company::factory()->for($this->workspace)->create(['name' => 'Untouched Co'])->delete();
...
test('an untouched rich editor is not reported as a change', function (): void {
    $company = Company::factory()->for($this->workspace)->create(['name' => 'Empty Editor Co']);
...
test('rich editor markup is read as its text, never as tags', function (): void {
    $company = Company::factory()->for($this->workspace)->create(['name' => 'Rich Editor Co']);
...
test('a row whose only change fits inline offers no slide-over', function (): void {
    $company = Company::factory()->for($this->workspace)->create(['name' => 'One Change Co']);
...
test('searching finds a record by the name it carries now', function (): void {
    Company::factory()->for($this->workspace)->create(['name' => 'Findable Holdings']);
    Company::factory()->for($this->workspace)->create(['name' => 'Unrelated Ltd']);
...
test('searching still finds a record that no longer exists', function (): void {
    $destroyed = Company::factory()->for($this->workspace)->create(['name' => 'Erased Industries']);
    $destroyed->delete();
    $destroyed->forceDelete();
...
test('a task row links to the modal that edits it', function (): void {
    $task = Task::factory()->for($this->workspace)->create(['title' => 'Renew the contract']);
...
test('searching never reaches into another workspace records', function (): void {
    ...
    Company::factory()->for($stranger->currentWorkspace)->create(['name' => 'Findable Holdings']);

    $this->actingAs($this->owner);
    Filament::setTenant($this->workspace);
    Company::factory()->for($this->workspace)->create(['name' => 'Findable Domestics']);
...
test('the slide-over stamps the change in the readers own timezone', function (): void {
    ...
    $company = Company::factory()->for($this->workspace)->create(['name' => 'Timezone Co']);
    $batch = (string) Str::uuid();
...
test('searching finds a record whose name was never written into a payload', function (): void {
    $silent = Company::factory()->for($this->workspace)->create(['name' => 'Silent Partners Ltd']);
    $unrelated = Company::factory()->for($this->workspace)->create(['name' => 'Unrelated Ltd']);
...
test('a long rich editor body does not ship whole into every row title', function (): void {
    $company = Company::factory()->for($this->workspace)->create(['name' => 'Long Copy Co']);
