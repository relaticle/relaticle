...
return [
     ...
     * if so the request will be stamped as spam.
     *
     * On, not off (the package default): with it off the check is skipped
     * entirely for a request carrying neither field, so a scripted POST that
     * simply omits them sails through and only a bot that bothers to fill the
     * hidden input is ever caught. This is not the global middleware; it is
     * applied to POST /contact and to the Filament registration page, and both
     * render <x-honeypot>, so nothing legitimate reaches here without the
     * fields. Adding a third protected form means adding the component to it.
     */
