...
final class ActivityLog extends Page implements HasTable
{
    ...
    protected string $view = 'filament.pages.workspace.activity-log';
    ...
    {
        return __('workspaces.tabs.activity');
    }
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->emptyStateHeading(fn (): string => $this->isFiltered()
                ? __('workspaces.activity.no_results.heading')
                : __('workspaces.activity.empty.heading'))
            ->emptyStateDescription(fn (): string => $this->isFiltered()
                ? __('workspaces.activity.no_results.description')
                : __('workspaces.activity.empty.description'))
            ->emptyStateActions([
                Action::make('clearFilters')
                    ->label(__('workspaces.activity.no_results.action'))
            ...
            ->columns([
                TextColumn::make('created_at')
                    ->label(__('workspaces.activity.columns.created_at'))
                    ->since()
                    ->dateTimeTooltip()
                    ->sortable(),
                ...
                TextColumn::make('subject_type')
                    ->label(__('workspaces.activity.columns.subject_type'))
                    ->color('gray')
                    ->icon($this->typeIcon(...))
                    ->formatStateUsing($this->typeLabel(...)),
                TextColumn::make('subject_id')
                    ->label(__('workspaces.activity.columns.record'))
            ...
            ->filters([
                SelectFilter::make('event')
                    ->label(__('workspaces.activity.filters.event'))
                    ->options($this->eventOptions())
                    ->query(fn (Builder $query, array $data): Builder => filled($data['value'] ?? null)
                        ? $query->whereIn('event', self::EVENT_ALIASES[$data['value']] ?? [$data['value']])
                        : $query),
                SelectFilter::make('subject_type')
                    ->label(__('workspaces.activity.filters.subject_type'))
                    ->options($this->typeOptions()),
                SelectFilter::make('causer')
                    ->label(__('workspaces.activity.filters.causer'))
                ...
                Filter::make('created_at')
                    ->schema([
                        ...
                        DatePicker::make('until')->label(__('workspaces.activity.filters.until')),
                    ])
    ...
    private function viewChangesAction(): Action
    {
        return Action::make('viewChanges')
            ->label(__('workspaces.activity.changes_modal.trigger'))
            ->slideOver()
    ...
    private function sameSave(QueryBuilder $sibling): QueryBuilder
    {
        return $sibling
            ...
            ->whereColumn('sibling.workspace_id', 'activity_log.workspace_id')
    ...
    private function destroyedNotice(Activity $record): ?string
    {
        ...
        return __('workspaces.activity.record_destroyed');
    }
    ...
    {
        return new HtmlString(view('filament.pages.workspace.activity-line', [
            'label' => $state['label'],
    ...
    private function dateIndicators(array $data): array
    {
        ...
        foreach (['from', 'until'] as $field) {
            ...
            $indicators[] = Indicator::make(__('workspaces.activity.filters.'.$field).': '.$value)
                ->removeField($field);
        }
    ...
    private function eventLabel(?string $state): string
    {
        ...
        if ($state === self::CUSTOM_FIELD_EVENT) {
            return __('workspaces.activity.events.updated');
        }
    ...
    {
        return [
            'created' => __('workspaces.activity.events.created'),
