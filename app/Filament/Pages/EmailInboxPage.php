...
final class EmailInboxPage extends Page
{
    ...
    {
        return __('filament/pages/email-inbox.navigation.label');
    }

    public static function getNavigationGroup(): string
    {
        return __('filament/pages/email-inbox.navigation.group');
    }
    ...
    protected function composeEmailAction(): Action
    {
        return Action::make('composeEmail')
            ...
            ->action(function (array $data): void {
                ...
                Notification::make()
                    ->title(__('filament/pages/email-inbox.notifications.queued.title'))
    ...
    protected function replyForwardEmailAction(): Action
    {
        return Action::make('replyForwardEmail')
            ...
            ->modalHeading(fn (array $arguments): string => match ($arguments['mode'] ?? 'reply') {
                'reply_all' => __('filament/pages/email-inbox.reply_forward.modal_heading.reply_all'),
                'forward' => __('filament/pages/email-inbox.reply_forward.modal_heading.forward'),
                default => __('filament/pages/email-inbox.reply_forward.modal_heading.reply'),
            })
            ->modalWidth(Width::SevenExtraLarge)
            ...
            ->action(function (array $data, array $arguments): void {
                ...
                Notification::make()->title(__('filament/pages/email-inbox.notifications.queued.title'))->success()->send();
            });
    }
    ...
    protected function manageSharingAction(): Action
    {
        return Action::make('manageSharing')
            ->label(__('filament/pages/email-inbox.manage_sharing.label'))
            ->icon('heroicon-o-lock-open')
            ->modalHeading(__('filament/pages/email-inbox.manage_sharing.modal_heading'))
            ->modalSubmitActionLabel('Save')
            ->schema([
                Select::make('privacy_tier')
                    ->label(__('filament/pages/email-inbox.manage_sharing.fields.privacy_tier.label'))
                    ->options(EmailPrivacyTier::class)
                    ->required(),

                Repeater::make('shares')
                    ->label(__('filament/pages/email-inbox.manage_sharing.fields.shares.label'))
                    ->defaultItems(0)
                    ->addActionLabel('Add teammate')
                    ->columns(2)
                    ->schema([
                        Select::make('shared_with')
                            ->label(__('filament/pages/email-inbox.manage_sharing.fields.shared_with.label'))
                        ...
                        Select::make('tier')
                            ->label(__('filament/pages/email-inbox.manage_sharing.fields.tier.label'))
                            ->options(EmailPrivacyTier::class)
                            ->required(),
                    ]),
            ])
            ...
            ->action(function (array $data, array $arguments): void {
                ...
                Notification::make()
                    ->success()
                    ->title(__('filament/pages/email-inbox.manage_sharing.notifications.saved.title'))
                    ->send();
            });
    }
        ...
        return Action::make('requestAccess')
            ...
            ->action(function (array $data, array $arguments): void {
                ...
                if (! $request instanceof EmailAccessRequest) {
                    Notification::make()
                        ->warning()
                        ->title(__('filament/pages/email-inbox.request_access.notifications.already_pending.title'))
                        ->send();
    ...
    private function composeFormSchema(): array
    {
        return [
            Grid::make(2)
                ->schema([
                    Select::make('connected_account_id')
                        ->label(__('filament/pages/email-inbox.fields.from.label'))
                    ...
                    Select::make('template_id')
                        ->label(__('filament/pages/email-inbox.fields.template.label'))
                        ->placeholder(__('filament/pages/email-inbox.fields.template.placeholder'))
            ...
            TagsInput::make('to')
                ->label(__('filament/pages/email-inbox.fields.to.label'))
                ->placeholder(__('filament/pages/email-inbox.fields.to.placeholder'))
                ->required()
                ->splitKeys(['Tab', ',', ' '])
            ...
            Grid::make(2)
                ->schema([
                    TagsInput::make('cc')
                        ->label(__('filament/pages/email-inbox.fields.cc.label'))
                        ->placeholder(__('filament/pages/email-inbox.fields.cc.placeholder'))
                        ->splitKeys(['Tab', ',', ' '])
                        ->suggestions(fn (): array => $this->contactEmailSuggestions()),

                    TagsInput::make('bcc')
                        ->label(__('filament/pages/email-inbox.fields.bcc.label'))
                        ->placeholder(__('filament/pages/email-inbox.fields.bcc.placeholder'))
                        ->splitKeys(['Tab', ',', ' '])
                        ->suggestions(fn (): array => $this->contactEmailSuggestions()),
                ]),
            ...
            RichEditor::make('body_html')
                ->label(__('filament/pages/email-inbox.fields.body.label'))
                ->required()
            ...
            Section::make('Privacy')
                ...
                ->schema([
                    Select::make('privacy_tier')
                        ->label(__('filament/pages/email-inbox.fields.privacy_tier.label'))
                        ->helperText(__('filament/pages/email-inbox.fields.privacy_tier.helper_text'))
                        ->options(EmailPrivacyTier::class)
            ...
            Section::make('Signature')
                ...
                ->schema([
                    Select::make('signature_id')
                        ->hiddenLabel()
                        ->placeholder(__('filament/pages/email-inbox.fields.signature.placeholder'))
    ...
    private function replyFormSchema(): array
    {
        return [
            ...
            RichEditor::make('body_html')
                ->label(__('filament/pages/email-inbox.fields.message.label'))
                ->required()
                ->mergeTags(EmailTemplateRenderService::MERGE_TAGS)
                ->toolbarButtons([
                    'bold', 'italic', 'underline', 'strike',
