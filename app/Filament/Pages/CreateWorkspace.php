...
final class CreateWorkspace extends RegisterTenant
{
    ...
    public function getCancelUrl(): ?string
    {
        ...
        if ($this->tenant instanceof Workspace) {
    ...
    {
        return $this->tenant instanceof Workspace
            ? __('filament/pages/workspaces.create_workspace.actions.go_to_workspace')
            : __('filament/pages/workspaces.create_workspace.actions.cancel');
    }
                    ...
                    ->submitAction(
                        Action::make('register')
                            ->label(fn (): string => $this->hasPendingInvites()
                                ? __('filament/pages/workspaces.create_workspace.actions.send_invites')
    ...
    {
        return Step::make(__('filament/pages/workspaces.create_workspace.steps.invite'))
            ->schema([
                Placeholder::make('invite_heading')
                    ->label(__('filament/pages/workspaces.create_workspace.headings.invite'))
                    ->hiddenLabel()
                    ->content($this->stepHeading(
                        __('filament/pages/workspaces.create_workspace.headings.invite'),
                        __('filament/pages/workspaces.create_workspace.headings.invite_description'),
                    ))
                    ->dehydrated(false),

                Placeholder::make('invite_subheading')
                    ->label(__('filament/pages/workspaces.create_workspace.headings.invite_subheading'))
                    ->hiddenLabel()
                    ->content(new HtmlString(
                        '<p class="text-sm font-medium text-gray-700 dark:text-gray-300">'
                        .e(__('filament/pages/workspaces.create_workspace.headings.invite_subheading'))
                        .'</p>'
                    ))
                    ->dehydrated(false),
                ...
                Repeater::make('invites')
                    ...
                    ->schema([
                        TextInput::make('email')
                            ->label(__('filament/pages/workspaces.create_workspace.form.invite_email_label'))
                            ->hiddenLabel()
                            ->email()
                            ->placeholder(__('filament/pages/workspaces.create_workspace.form.invite_email_placeholder'))
                        ...
                        Select::make('role')
                            ->label(__('filament/pages/workspaces.create_workspace.form.invite_role_label'))
                            ->hiddenLabel()
                            ->options([
                                WorkspaceRole::Editor->value => __('filament/pages/workspaces.create_workspace.form.invite_role_member'),
                                WorkspaceRole::Admin->value => __('filament/pages/workspaces.create_workspace.form.invite_role_admin'),
                            ])
                    ...
                    ->addActionLabel(__('filament/pages/workspaces.create_workspace.actions.add_more')),

                Actions::make([
                    Action::make('copyInviteLink')
                        ->label(__('filament/pages/workspaces.create_workspace.actions.copy_invite_link'))
                        ...
                        ->action(function (): void {
                            if (! $this->tenant instanceof Model) {
                                try {
                                    ...
                                    Notification::make()
                                        ->title(__('filament/pages/workspaces.create_workspace.notifications.complete_previous_steps.title'))
                                        ->body(__('filament/pages/workspaces.create_workspace.notifications.complete_previous_steps.body'))
                                        ->warning()
                                        ->send();
                                ...
                                $this->tenant = resolve(CreateWorkspaceAction::class)->create($user, $data);
                            ...
                            Notification::make()
                                ->title(__('filament/pages/workspaces.create_workspace.notifications.invite_link_copied.title'))
                                ->body(__('filament/pages/workspaces.create_workspace.notifications.invite_link_copied.body'))
                                ->success()
                                ->send();
                        }),
    ...
    protected function afterRegister(): void
    {
        ...
        // Flagged here rather than at either CreateWorkspaceAction call site: this
    ...
    protected function handleRegistration(array $data): Model
    {
        ...
        // in the wizard, which pre-creates the workspace so the invite URL can exist.
        ...
        if ($this->tenant instanceof Workspace) {
            ...
            $updates = array_filter(
                ...
                fn (?string $value, string $key): bool => $value !== null && $workspace->{$key} !== $value,
            ...
            /** @var Workspace $workspace */
            $workspace = resolve(CreateWorkspaceAction::class)->create($user, $data);
        }

        $this->sendOnboardingInvites($user, $workspace, $data);
     ...
     * @param  array<string, mixed>  $data
     */
    private function sendOnboardingInvites(User $user, Workspace $workspace, array $data): void
    {
        ...
        foreach ($rawInvites as $invite) {
            ...
            if (filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
                $failed[] = [
                    'email' => $email,
                    'reason' => __('filament/pages/workspaces.create_workspace.notifications.some_invites_failed.invalid_email'),
                ];
            ...
            if ($transportIsDown) {
                $failed[] = [
                    'email' => $email,
                    'reason' => __('filament/pages/workspaces.create_workspace.notifications.some_invites_failed.send_skipped'),
                ];
            ...
            try {
                resolve(InviteWorkspaceMember::class)->invite(
                    $user,
                    $workspace,
                    $email,
                    $invite['role'] ?? WorkspaceRole::Editor->value,
                );
                ...
                $failed[] = [
                    ...
                    'reason' => is_string($firstError)
                        ? $firstError
                        : __('filament/pages/workspaces.create_workspace.notifications.some_invites_failed.generic'),
                ];
                ...
                $failed[] = [
                    'email' => $email,
                    'reason' => __('filament/pages/workspaces.create_workspace.notifications.some_invites_failed.send_failed'),
                ];
            }
        }
