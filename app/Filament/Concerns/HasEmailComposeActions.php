...
trait HasEmailComposeActions
{
    ...
    protected function composeEmailAction(): Action
    {
        return Action::make('composeEmail')
            ->label(__('filament/concerns/email-compose.compose.label'))
            ->slideOver()
    ...
    protected function replyForwardEmailAction(): Action
    {
        return Action::make('replyForwardEmail')
            ...
            ->modalHeading(fn (array $arguments): string => match ($arguments['mode'] ?? 'reply') {
                'reply_all' => __('filament/concerns/email-compose.reply_forward.modal_heading.reply_all'),
                'forward' => __('filament/concerns/email-compose.reply_forward.modal_heading.forward'),
                default => __('filament/concerns/email-compose.reply_forward.modal_heading.reply'),
            })
    ...
    private function sendQueuedNotification(Email $email): void
    {
        ...
        if ($email->scheduled_for !== null && $email->scheduled_for->isFuture()) {
            $notification->actions([
                Action::make('undo')
                    ->label(__('filament/concerns/email-compose.notifications.undo_action.label'))
                    ->link()
                    ->action(function () use ($email): void {
                        try {
                            resolve(CancelQueuedEmailAction::class)->execute($email->refresh());
                            Notification::make()->title(__('filament/concerns/email-compose.notifications.send_cancelled.title'))->success()->send();
                        } catch (RuntimeException) {
                            Notification::make()->title(__('filament/concerns/email-compose.notifications.send_too_late.title'))->danger()->send();
                        }
                    }),
            ]);
        }
