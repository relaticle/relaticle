{{-- Crabdev: copy of upstream's filament/pages/create-team.blade.php with our name
     in the footer and without Relaticle's privacy and terms links. --}}
<div class="flex min-h-full flex-col">
    ...
    <div class="flex items-center justify-center gap-x-1 py-6 text-xs text-gray-400 dark:text-gray-500">
        <span>&copy; {{ date('Y') }} {{ config('crm.brand.company') }}</span>
        <span>&middot;</span>
        <form method="POST" action="{{ filament()->getLogoutUrl() }}" class="inline">
            @csrf
            <button type="submit" class="hover:text-gray-600 dark:hover:text-gray-300">{{ __('Sign out') }}</button>
        </form>
    </div>
</div>
