...
{{-- One row inside a mega-menu panel. Renders a brand icon tile plus a
     ...
     Icons migrated to the isocons.app set live as their own component under
     `components/icons/`; anything not yet migrated (e.g. `blog`) still
     resolves through the legacy `x-brand.nav-icon` name-keyed set. --}}
...
<a href="{{ $item->url }}"
   ...
   class="group flex items-start gap-3 rounded-lg px-3 py-2 hover:bg-gray-50 dark:hover:bg-white/[0.04] transition-colors">
    ...
    @if($item->icon)
        <span class="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-gray-100 text-gray-500 ring-1 ring-inset ring-gray-900/[0.04] group-hover:bg-primary/[0.1] group-hover:text-primary group-hover:ring-primary/20 dark:bg-white/[0.07] dark:text-gray-400 dark:ring-white/[0.07] dark:group-hover:bg-primary/25 dark:group-hover:text-primary-300 dark:group-hover:ring-primary/30 transition-colors">
            @if(in_array($item->icon, \App\Support\NavItem::ICON_COMPONENTS, true))
                <x-dynamic-component :component="'icons.'.$item->icon" class="h-7 w-7"/>
            @else
