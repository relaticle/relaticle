            ...
            @foreach ($pendingAccessRequests as $accessRequest)
                <div class="flex items-center justify-between gap-3 rounded-lg border border-amber-200 bg-white px-3 py-2 dark:border-amber-800 dark:bg-gray-900">
                    ...
                    <div class="flex shrink-0 items-center gap-1.5">
                        <button
                            @if ($inlineAccessActions)
                                x-on:click="window.Livewire.dispatchTo(@js(\Relaticle\EmailIntegration\Livewire\EmailAccessNotificationHandler::LIVEWIRE_ALIAS), 'approve-email-access-request', { requestId: @js($accessRequest->id) })"
                            @else
                        ...
                        <button
                            @if ($inlineAccessActions)
                                x-on:click="window.Livewire.dispatchTo(@js(\Relaticle\EmailIntegration\Livewire\EmailAccessNotificationHandler::LIVEWIRE_ALIAS), 'deny-email-access-request', { requestId: @js($accessRequest->id) })"
                            @else
