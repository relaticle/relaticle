<div class="space-y-4">
    ...
    @if ($this->pendingSecret)
        <div class="space-y-4 rounded-lg border border-gray-200 p-4 dark:border-gray-700">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">
                {{ __('profile.sections.mfa.manual_hint') }}
                <code class="ml-1 rounded bg-gray-100 px-2 py-1 font-mono text-xs text-gray-900 dark:bg-gray-800 dark:text-gray-100">{{ $this->pendingSecret }}</code>
            </p>
    ...
    @if ($this->enabled)
        <div class="space-y-3 rounded-lg border border-gray-200 p-4 dark:border-gray-700">
            ...
            @if ($this->revealedRecoveryCodes !== [])
                <ul class="grid gap-2 rounded-lg bg-gray-50 p-4 font-mono text-sm text-gray-900 sm:grid-cols-2 dark:bg-gray-800 dark:text-gray-100">
