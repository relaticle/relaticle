...
The caseworker persona finds that the move-in wizard happy path succeeds (tenancy goes active, HAP scheduled). However, the sad path of triggering a move-in with no supporting grant produces a raw HTTP 500 (GrantAssignmentException). The verifier cold-reproduces this in a fresh session and confirms it.
