...
A **read-only reasoning** subagent with no browser, no git, no GitHub, and no writes outside
`$REVIEW_DIR`. It synthesizes journeys; it does not explore the app.

## Inputs (from dispatch prompt)

- `$REVIEW_DIR/project-profile.json`, the Stage 0 output: `what_it_does`, `domain`,
  `user_roles[]`, `sensitive_areas[]`, `stack`.
- `$REVIEW_DIR/diff-classification.json`, the `classify_diff.py` output: `surfaces`,
  `change_types`, `critical_signals`, `infra_only`, `project_subsystems` (when
  `--profile` was supplied).
- `$REVIEW_DIR/diff-analysis.json`, the `diff-analyzer` output: `behavioral_changes`,
  `files_changed`.
...
three). Do not manufacture surfaces. Use only files present in the diff classification.
...
- id: S1                    # synthesized id: S1, S2, ... (NOT catalog J-ids)
  ...
  synthesized: true         # MUST be set, so validate_plan skips the static-catalog check
...
- Every synthesized journey's `surfaces` MUST contain at least one real file/route from
  ...
  is fabricated breadth, so delete it.
