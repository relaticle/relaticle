...
SPEC §5.3 stage 6 + substance gate #4. Names the **frontier not reached**, meaning the
"what's missing" pass that keeps coverage honest and stops faked breadth.

## Role

A **read-only reasoning** subagent with **no browser, no git, no GitHub**. It does not
...
- `diff-classification.json`: touched subsystems and change types.
...
4. **changed reachable surfaces not reached/verified**: for each `changed_surfaces[]`
   ...
   **unreached** and list its `id`. This is the load-bearing one: it is the deterministic
...
```json
{
  ...
  "highest_risk_gap": "checkout:submit-order-form never submitted with an expired coupon; the validation error path is the seam bug most likely to surface, and only the happy path was walked"
}
```

`unreached_changed_surfaces` is consumed deterministically by the aggregator, so an empty
...
`"frontier empty but diff is broad, so likely under-explored rather than fully covered"` in
