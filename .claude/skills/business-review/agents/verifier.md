...
bugs from scratch**. Browser-only, with **no git, no GitHub**, and no other skills except
...
it cannot confirm anything, so its confirmations are unreliable. **STOP** and flag
