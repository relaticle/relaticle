...
- **Shared local Redis lets ANOTHER Herd app's Horizon eat this app's queue jobs.**
  (verified 2026-06-11, when Journey's Horizon consumed Relaticle chat jobs; ~90 min lost,
...
- **agent-browser's Chromium may not resolve Herd's Reverb vhost / wrong websocket
  host.** (verified 2026-06-10 on PR 322, where it looked like a degraded channel but was a stale JS
...
- **Sync queue silently invalidates queued-flow verification** (`release()`, retries,
  chain caps untestable). (verified 2026-06-09 on PR 321, where the user mandated an async re-run.)
...
- **Subagent capacity deaths mid-run.** (verified 2026-06-11/12, when code-context-analyzer
...
- **Self-contamination**: a leftover event listener / auto-approve hook from earlier
  ...
  500. (verified 2026-06-11/12, when both happened.) Avoid: fresh session per case where
...
- **Modal fade ~300ms**, so don't assert removal immediately; wait on state.
  (verified: 2026-05.)
