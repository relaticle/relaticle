...
critique, which is `design-review`'s job and an explicit non-goal here. It does **NOT**
...
A persona runs this lens **continuously** as it walks a journey, so every friction
...
behavior the persona saw, never an aesthetic opinion.
...
2. **Missing empty / loading / error states.** A blank screen instead of "no
   ...
   something fails.
3. **Error copy that is not human or not recoverable.** "Error 500" or a raw stack
...
6. **Destructive action with no confirm, or a confirm with no undo.** Delete or discard
...
7. **Silent failure.** An action appears to do nothing: no error, no success, no
   change. (The worst kind, because the user can't tell whether it worked.)
...
- A **`high`-severity** friction item is a strong signal: it **pushes the report's
  judgment toward `ai-needs-human`** even when no functional blocker exists, because a PM
