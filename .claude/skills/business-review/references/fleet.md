...
- **Personas (parallel subagents).** Each plays ONE persona, drives its **own** browser
  session, and does **browser-only** work and **never touches git**. Subagents that read
...
**Tier 0 collapses to a single in-process persona pass**, with no subagent spawn. One
...
1. **happy-path first**, establishing that the value arc works. For a journey that requires
...
3. **explore each journey to convergence**, going until no *new* behavior surfaces,
   then stop.
...
surfaced for adversarial verification, never silently skipped. Run the **UX lens
...
The orchestrator reads **only the findings JSON**, never persona browser transcripts.
...
state is unchanged, and moves on. It never commits.
