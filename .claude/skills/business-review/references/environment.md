...
3. `.env` (`APP_URL`, `*_DOMAIN` keys) read directly, because the config cache may be stale; if
...
4. Nothing works → credentials **Context gap** (end-of-Stage-1 question or
   `ai-needs-human`), **never `blocked`**, because the browser channel is fine.
...
- **Queue-ownership sentinel probe** (the false-signal killer; see gotchas on shared local
...
`heals[]` lists every cached file updated this run, and the report's Notes section surfaces
