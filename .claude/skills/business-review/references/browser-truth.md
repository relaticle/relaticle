...
3b. **Establishing a login is SETUP, not a "fix."** Authentication is the most basic
   ...
   **production snapshot**), bootstrap one test login via the project's OWN tooling, meaning a
   ...
   at worst `ai-needs-human`; it is **never** a degraded channel or `blocked`, because the browser
