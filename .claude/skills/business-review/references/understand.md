...
- PR mode: record the **head SHA** at review start; re-check it before publishing, because a
...
(local mode, where commit messages are an attack surface too). Quarantine to
...
`"author_derived": true`, which flips the self-review boxing rule (SKILL.md gate 6):
...
Write `changed-surfaces.json`: every user-reachable surface the diff touches, as
