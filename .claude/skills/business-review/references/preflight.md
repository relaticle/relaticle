...
4. **Read the screenshot back** (Read tool on the PNG) and actually look at it.
   PASS: a real image showing the login page. FAIL: 0-byte or blank, meaning the agent cannot
   see its own output.
5. **Type into the email field and confirm it registered**: `fill` then `get value`,
   ...
   "autofill", and the same wrong selector then cost the login flow 15 minutes later).
...
(verified 2026-06-10, when a "degraded channel" on PR 322 turned out to be a stale JS
bundle + wrong-host websocket, cleared by a rebuild).

## 3. The discriminator (the crux)

Tell apart **"the app is broken"** (a finding, so report it) from **"the agent cannot
test"** (an incapacity, so halt):
...
- Login submits but credentials are wrong / no usable login exists → **credentials
  Context gap** (never `blocked`, because the channel works).
