...
| **P** | Product | Internal consistency | CompanyResource sorts by name asc by default; PeopleResource sorts by created_at desc, same table component with a different default |
...
| 4 | Are all icons the correct variant per CLAUDE.md icon rules, so UI/functional = `line`, brand/social = `fill`, status/emphasis = `fill`? | Medium |
| 5 | Is spacing between sibling elements visually consistent (no surprise gaps, no cramped pairs)? | Medium |
| 6 | Are colors drawn from the design tokens, with no hardcoded hex that bypasses the theme? | Medium |
...
| 12 | Are aligned items actually aligned across table columns, form labels, button rows, and card grids? | Medium |
