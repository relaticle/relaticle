...
2. `GET /v1/<resource>/<id>` via REST, then assert the payload matches.
3. Call `Show<Resource>Tool` via MCP, then assert the payload matches.
4. Export to CSV (if the resource supports it), then assert columns and values.
5. (Optional) Re-import the CSV, then assert no duplication, no truncation, no encoding loss.
...
3. Cross-tenant probe each: as Team A, attempt to read Team B's encrypted field via API/MCP. Expect 403 or zero data.
...
**Relaticle steps:**
1. Open a multi-step form. Start filling. Hit browser back and return. Is the form draft preserved or cleanly cleared?
...
4. System administrator panel, if the `/sysadmin/*` routes are accessible.
...
1. **Viewport sweep.** Set the viewport to each of `320×568`, `375×812`, `768×1024`, `1024×768`, `1440×900`. At each: open the page, `agent-browser wait --load networkidle`, sleep 1s, run probes P1–P8 from `references/visual-probes.md`, take `agent-browser screenshot --annotate`. Skip-rules per viewport are in `visual-probes.md`'s viewport sweep table.
...
**Always-include rule:** Supermodel is **mandatory** for any matrix that contains a Filament UI or Livewire surface, regardless of risk tier. The tier-cap on tour count (smoke=1, light=2, …) does not apply, because Supermodel does not count against that cap. Reason: the diffs most likely to ship visual bugs (CSS, Blade, Tailwind config, view-component changes) score smoke or light on the risk rubric, and skipping the only visual tour at those tiers is exactly when bugs slip through.
