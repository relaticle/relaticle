# Custom Fields 4.0, Plan 3.1: Relationship Substrate
...
**Spec:** docs/superpowers/specs/2026-09-03-cf4-p3-relationships-substrate-spec.md (in relaticle/relaticle workspace; this plan implements spec sections 1 and 2)

**Repo:** work happens in the `~/Herd/custom-fields-4x` worktree on branch `4.x`, cut from `origin/3.x` by Plan 1.1 Task 0 (there is no `main` in the package). Never work on the existing checkout's branch.

## Amendments recorded during execution (2026-09-06)

The codebase wins where it and this plan disagree; each item below is the code's shape.

- Migration files are `create_relationship_definitions_table.php` and
  `create_relationship_links_table.php`: Laravel sorts migrations by name, and the plan's
  `create_custom_field_*` names sorted before `create_custom_fields_table`, so the foreign keys
  had no target on Postgres and MySQL.
- `active_from` and `active_until` are `dateTime` columns, not `timestamp`: MySQL gives the
  first NOT NULL timestamp column an implicit `CURRENT_TIMESTAMP ON UPDATE` default.
- `RelationshipCardinality` implements `HasLabel` through `__()` (the arch rule for enums);
  there is no `label()` method.
- Data objects are camelCase behind the snake mapper: `RelationshipDefinitionData(code:,
  fromEntityType:, toEntityType:, cardinality:, isSymmetric:, fromField:, toField:)` and
  `FieldSlotData(name:, sectionId:, fieldId:)`; the sample payloads below use the old
  `from_entity_type:` keys and should be read with that mapping.
- `LinkWriter::apply(Model $record, CustomField $field, array $ids, ...)` takes a plain
  `Model` and an id array (or the `['ids' => [...], 'replace' => true]` map through
  `RecordLinkPayload`); null is coerced to `[]` by the trait.
- `DeleteRelationshipDefinition` deletes links explicitly because SQLite runs with foreign
  keys off in the suite; the FK cascade still applies on Postgres and MySQL.
- `SYSTEM_RELATIONSHIPS` gates the two relationship migrations, the `lookup_type` drop, and
  the upgrade steps only; a field with a definition reads and writes links regardless of
  the flag (`Support\RelationshipTables::exist()` is the runtime check).
- An absent key in a payload leaves a definition-backed field's links untouched; explicit
  `[]` or null clears them (the host's documented contract, see the upgrade guide).
- The write path wraps custom-field persistence in a transaction from the trait's `save()`
  so a rejected link rolls the record back.
...
## Cardinality enforcement (spec 1.2)

Per-end partial unique indexes are not implementable statically: which end is constrained varies per definition row, and per-definition indexes would be dynamic DDL, which the spec forbids. The 2026-09-01 architecture review replaced them with the two layers below, and spec 1.2 records that decision. Plan 3.1 enforces:
...
DB-trigger enforcement can be added later without schema change.

---

### Task 1: Config keys and Cardinality enum
...
- Produces: `CustomFieldsFeature::SYSTEM_RELATIONSHIPS` (default enabled, host-disableable like the other SYSTEM_* flags); the two new migrations and the record-field write fork are gated on it exactly as `SYSTEM_MULTI_TENANCY` gates the tenant column.
- Produces: config key `custom-fields.database.key_type` (`bigint` default, `ulid`, `uuid`) plus a small blueprint helper (`KeyType::primary($table)`, `foreign($table, $column)`, `morphs($table, $name)`) that Tasks 2 and 3 use for the two NEW tables. Existing tables keep the publish-and-edit path; the helper ends the hand edit for the new ones and is the seam that later majors can extend to all tables.

- [ ] **Step 1: Confirm the worktree**

```bash
git -C ~/Herd/custom-fields worktree list | grep custom-fields-4x
git -C ~/Herd/custom-fields-4x log --oneline -1
```

Plan 1.1 Task 0 created it and Plans 1.1 and 2.1 have already landed on it. If it is missing, run Plan 1.1 Task 0 first; never recreate the branch here.
...
**Files:**
- Create: `database/migrations/create_relationship_definitions_table.php` (follow the publish pattern of `database/migrations/create_custom_fields_table.php`)
...
Migration `database/migrations/create_relationship_definitions_table.php` (same anonymous-class + feature-flag shape as the existing create migration):
...
**Files:**
- Create: `database/migrations/create_relationship_links_table.php`
...
Migration `create_relationship_links_table.php`:
...
**Interfaces:**
- Produces: `interface LinkActorResolver { public function resolve(): ?\Illuminate\Database\Eloquent\Model; }`. Default returns `auth()->user()`. Hosts rebind (Relaticle binds an agent-aware resolver in Plan 5.1).
...
- Deliberately out of Plan 3.1: substrate-independent housekeeping runs BEFORE this plan (Plan 1.1); definitions management UI, creation modal, flavor registry (Plan 4.1); lookup_type retirement and the reflection-helper deletion (Plan 3.2, after T12 has migrated data; it also rebuilds FieldForm's record config on definitions with stock Filament); Relaticle bindings, timeline listener, GetRelatedRecordsTool, path-repo wiring (Plan 5.1).
