...
Spec 1.2 says cardinality is enforced with per-end partial unique indexes. That is not implementable statically: which end is constrained varies per definition row, and per-definition indexes would be dynamic DDL, which the spec itself forbids. Plan 3.1 therefore enforces:
...
Plan 1.1 runs in this same worktree first; if it already exists, skip creation.
