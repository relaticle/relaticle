...
- Deliberately out of Plan 3.1: substrate-independent housekeeping runs BEFORE this plan (Plan 1.1); definitions management UI, creation modal, flavor registry (Plan 4.1); lookup_type retirement and the reflection-helper deletion (Plan 3.2, after T12 has migrated data and Plan 4.1 has replaced FieldForm's record path); Relaticle bindings, timeline listener, GetRelatedRecordsTool, path-repo wiring (Plan 5.1).
