...
> **For agentic workers:** REQUIRED SUB-SKILL: sdd-lean, task-by-task. Runs on the `4.x` branch AFTER Plan 1.1 (clean base, interface break landed) and BEFORE Plan 4.1 Task 4 and Plan 4.3 (the field-management polish pass and the bulk-paste action reshape the same options repeater; land the category column first so those restyle it once). Plan 2.2 consumes it. Substrate-independent: nothing here reads lookup_type, definitions, or links. Every task ends with the package quality gate: `vendor/bin/pint --dirty --format agent`, `vendor/bin/rector --dry-run`, `vendor/bin/phpstan analyse`, `vendor/bin/pest --type-coverage --min=100.0`, targeted pest run.
