# Custom Fields 4.0, Plan 5.1: Relaticle Surface Migration

> **For agentic workers:** REQUIRED SUB-SKILL: sdd-lean, task-by-task. Runs in the relaticle/relaticle repo on a feature branch, AFTER package phases 1 through 4 are complete and tagged on the `4.x` branch (spec: 2026-09-03-cf4-p5-host-adoption-and-release-spec.md), with the package wired via composer path repository until 4.0.0 tags. Relaticle gates apply (pint, rector, phpstan level 7, type coverage 100, targeted pest; `composer test:pest:full` before push).
...
**Two link types (plan 4.4):** the package ships `record` (the one-way link, unchanged for users) and `relationship` (paired, with cardinality, symmetry and chips). Every host surface detects a link field by its relationship definition, never by the key, so both types are covered by the same change: `relationshipDefinition() !== null`, or `typeData->requiresRelationship`, never `type === 'record'`. Sweep chat, REST, MCP and import for a hardcoded `'record'` before closing Task 3 and Task 5. The chat schema describes a relationship field with its cardinality and, where it is paired, the field on the other entity, so the assistant can say what a write to it will do on both records.
