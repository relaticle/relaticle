...
- 27 stale remote feature branches predate 3.x. After confirming each is merged or abandoned, delete them alongside the stale `4.x`/`5.x` (Plan 3.1 Task 1 covers those two). Optional, but it removes noise from every `git fetch` for the rest of the program.
