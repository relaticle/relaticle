# Custom Fields 4.0, Plan 4.2: Through-Relation Table Surfaces

> **For agentic workers:** REQUIRED SUB-SKILL: sdd-lean, task-by-task. Runs on the `4.x` branch AFTER Plan 3.1 (substrate) and Plan 1.1 Task 1 (which carries the `?string $through` parameter on `TableFilterInterface`). Independent of Plan 4.1; either order. Package quality gate per task: `vendor/bin/pint --dirty --format agent`, `vendor/bin/rector --dry-run`, `vendor/bin/phpstan analyse`, `vendor/bin/pest --type-coverage --min=100.0`, targeted pest.

**Goal:** Implement section 3.5 of the phase 4 spec (2026-09-03-cf4-p4-relationship-ux-spec.md). Let a Filament table whose row record does not itself hold custom fields display, filter and sort the custom fields of a to-one related record.
...
**Why it waits for Plan 3.1:** Plan 3.1 rewrites `RecordColumn` sorting/searching and `RecordFilter` onto link-backed `whereExists`. Building through-relation support against `json_value` first would mean building the same three query paths twice.
...
- **No new interface parameters beyond Plan 1.1 Task 1.** Columns are re-wrapped from `TableBuilder` using idempotent public setters. Filters use the `$through` parameter that Plan 1.1 already added.
- Package conventions per Plan 3.1's global constraints: `strict_types`, final, fully typed, config-driven table and column names.
...
- Pass the through path into `FieldFilterFactory::create()` and on to each filter's `make(..., ?string $through)`, the parameter Plan 1.1 Task 1 added. This is why filters differ from columns: `modifyQueryUsing` is protected with no public getter and each filter type builds a different query, so the builder cannot wrap what it cannot read.
