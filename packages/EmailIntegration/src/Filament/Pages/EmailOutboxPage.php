...
final class EmailOutboxPage extends Page implements HasTable
{
    ...
    {
        return __('filament/pages/email-outbox.navigation.group');
    }
        ...
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('participants_to')
                    ->label(__('filament/pages/email-outbox.columns.participants_to.label'))
                    ->state(fn (Email $record): string => $record->participants
                        ->where('role', 'to')->pluck('email_address')->implode(', ')),
                TextColumn::make('status')->badge(),
                TextColumn::make('scheduled_for')->dateTime()->label(__('filament/pages/email-outbox.columns.scheduled_for.label')),
            ...
            ->recordActions([
                Action::make('cancel')
                    ...
                    ->action(function (Email $record): void {
                        ...
                        Notification::make()->title(__('filament/pages/email-outbox.notifications.cancelled.title'))->success()->send();
                    }),
                Action::make('reschedule')
                    ...
                    ->schema([
                        DateTimePicker::make('scheduled_for')
                            ->label(__('filament/pages/email-outbox.fields.scheduled_for.label'))
                            ->seconds(false)
                            ->minDate(now())
                            ->required(),
                    ])
                    ...
                    ->action(function (Email $record, array $data): void {
                        ...
                        Notification::make()->title(__('filament/pages/email-outbox.notifications.rescheduled.title'))->success()->send();
                    }),
                Action::make('retry')
                    ...
                    ->action(function (Email $record): void {
                        ...
                        Notification::make()->title(__('filament/pages/email-outbox.notifications.retry_queued.title'))->success()->send();
                    }),
            ])
            ->toolbarActions([
                BulkAction::make('bulkCancel')
                    ->label(__('filament/pages/email-outbox.actions.bulk_cancel.label'))
                    ->color('danger')
                    ->icon(Heroicon::OutlinedXMark)
                    ->requiresConfirmation()
                    ->action(function (Collection $records): void {
                        ...
                        Notification::make()->title(__('filament/pages/email-outbox.notifications.bulk_cancelled.title', ['count' => $cancelled]))->success()->send();
                    }),
            ]);
    }
