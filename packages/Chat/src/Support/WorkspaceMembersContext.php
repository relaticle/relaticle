...
/**
 * Workspace member directory for the chat tools: backs ListWorkspaceMembersTool and
 * validates workspace-member fields (company account_owner_id, task assignee_ids),
 * which accept ONLY workspace users, never CRM contacts.
 */
final readonly class WorkspaceMembersContext
{
    ...
    /**
     * Validate a workspace-member field value at proposal time: null and '' pass
    ...
    public static function memberFieldError(User $user, string $field, mixed $value): ?string
    {
        ...
        $known = $workspace->users()->whereKey($ids)->pluck('users.id')->map(strval(...))->all();
        $known[] = (string) $workspace->user_id;

        if (array_diff($ids, $known) === []) {
            return null;
        }

        return "{$field} must be a workspace member. Valid members: "
